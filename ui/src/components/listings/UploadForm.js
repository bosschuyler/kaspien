...
class UploadForm extends React.Component {
  ...
  validate() {
    ...
    // Add with the notes/labeling for upload
    // if (!data.name) {
  ...
  getForm() {
    ...
    return (
      <form onSubmit={this.submit.bind(this)}>
        ...
        <div className="mb-3">
          <label htmlFor="csv-product-uploader">CSV File</label>
        ...
        <button type="submit" disabled={was_validated && (submitting || invalid)} className="btn btn-primary btn-spaced">
          {submitting ?
            <Loading>Submitting...</Loading> :
            <div>Submit</div>
          }
        </button>
        <button type="button" onClick={this.back} disabled={submitting} className="btn btn-danger btn-spaced">
          <div>Cancel</div>
        </button>
  ...
  render() {
    ...
    if (initializing) {
      ...
      Content = <Notice type={warnings.length ? 'warning' : 'success'} title={success} messages={warnings}>
        <button onClick={this.reload} className='btn btn-primary btn-spaced'><i className="fa fa-plus"></i> Upload New File</button>
        <button onClick={this.back} className='btn btn-danger'>Cancel</button>
      </Notice>;
    } else {
